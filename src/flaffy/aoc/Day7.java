...
public class Day7 {
    ...
    // possibleWrappings wird nur einmal initialisiert, deshalb können nicht mehrere
    // Inputs in einem Durchlauf ausgewertet werden. (TODO)
    ...
    public int calc1(List<String> allLines) {
        recursiveCheck(allLines, "shiny gold");
        return possibleWrappings.size();
    }

    List<String> possibleWrappings = new LinkedList<String>();

    public void recursiveCheck(List<String> allLines, String color) {
    ...
    // auswerten: die ersten 2 Wörter (äußere Farbe), die 2 Worte nach jeder Zahl
    // (erlaubte Inhalte),
    ...
    public String analyse(String line, String color) {
        ...
        String[] contents = wrappingAndContent[1].split(" bag[s]?(,[\s]|[.])"); // Inhalte voneinander teilen (jeweils
                                                                                // Zahl und Farbe in einer Zelle)
        String[][] contents2 = new String[contents.length][2]; // (Zahlen von Farben trennen)
        for (int i = 0; i < contents.length; i++) { // wenn color in Inhalten vorkommt, wird Verpackungsfarbe
                                                    // zurückgegeben.
    ...
    private class Line {
        ...
        public Line(String line) {
            ...
            String[] mixedContent = line.split(" bags contain ")[1].split(" bag[s]?(,[\s]|[.])"); // Inhalte voneinander
                                                                                                  // teilen (jeweils
                                                                                                  // Zahl und Farbe in
                                                                                                  // einer Zelle)
